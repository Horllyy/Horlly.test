...
/**
 * 功能：
 * excel2003 .xls格式，以及excel2007 .xlsx格式的表格复制
 *
 *
 * 我怎么感觉2003和2007的代码一样啊。。是不是唬我啊妈的
 *
 * 下午17：35测试
 * 奶丝啊 不管怎么着 我没比对过是不是一样 但是.xlsx格式的用2007可以读取
 * 这段代码可行 可行就好 管它一不一样。。。
 * 快乐 告辞！
 */
public class ExcelCopy {
    ...
    public static void main(String[] args) throws IOException {
        File file = new File("D:\\trial\\Excel1.xlsx");
        ArrayList<ArrayList<Object>> result = ExcelCopy.readExcel(file);
        for(int i = 0 ;i < result.size() ;i++){
            for(int j = 0;j<result.get(i).size(); j++){
                System.out.println(i+"行 "+j+"列  "+ result.get(i).get(j).toString());
            }
        }
        ExcelCopy.writeExcel(result,"D:\\trial\\Excel1Copy.xlsx");
    }
